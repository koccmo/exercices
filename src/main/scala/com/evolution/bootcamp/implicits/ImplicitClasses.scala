...
object ImplicitClasses {
  ...
  object EvolutionUtils0 {
    ...
    def pow(base: Int, exponent: Int): Int = Math.pow(base, exponent).round.toInt
    ...
    def mean(list: List[Int]): Int = list match {
      case _ :: _        => list.sum / list.size
  ...
  object EvolutionUtils1 {
    def int_pow(base: Int, exponent: Int): Int            = Math.pow(base, exponent).round.toInt
    def int_concat(a: Int, b: Int): Int                   = (a.toString + b.toString).toInt
    def string_toInstant(string: String): Option[Instant] = Try(Instant.parse(string)).toOption
    def list_int_mean(list: List[Int]): Int               = list match {
      case _ :: _        => list.sum / list.size
      case number :: Nil => number
      case _             => 0
    }
  ...
  object EvolutionUtils3 {
    ...
    case class RichInt(a: Int)              {
      def pow(exponent: Int): Int = Math.pow(a, exponent).round.toInt
      def concat(b: Int): Int     = (a.toString + b.toString).toInt
    }
