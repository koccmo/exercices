...
object ImplicitParameters {
  ...
  object EvolutionUtils0 {
    ...
    class CreditService {
      ...
      def credit(context: WalletContext, amount: BigDecimal): Unit = context.read.fold {
        context.create
        context.update(amount)
      } { balance => context.update(balance + amount) }
    }
    class DebitService {

      /** Removes money from wallet */
      def debit(context: WalletContext, amount: BigDecimal): Unit = context.read.fold {
        context.create
        context.update(-amount)
      } { balance => context.update(-balance) }
    }
    class TransferService(creditService: CreditService, debitService: DebitService) {
      ...
      def transfer(context: WalletContext, amount: BigDecimal): Unit = if (amount > 0) {
  ...
  object EvolutionUtils1 {
    ...
    class CreditService                                                             {
      def credit(amount: BigDecimal)(context: WalletContext): Unit = context.read.fold{ context.create
      context.update(amount) } { balance => context.update(balance + amount) }
    }
    class DebitService                                                              {
      def debit(amount: BigDecimal)(context: WalletContext): Unit = context.read.fold{ context.create
      context.update(-amount) } { balance => context.update(balance - amount) }
    }
    class TransferService(creditService: CreditService, debitService: DebitService) {
      def transfer(amount: BigDecimal)(context: WalletContext): Unit = if (amount > 0) creditService.credit(amount)(context)
